...
# high ratio means the quantile is cloer to the covariate quantile, so we sort decreasing = False
or <- order(rat, decreasing = FALSE) # rank the index where the biggest discrepancy is
...
# rp <- l1[indy, 1]  # row index of pixel
# rc <- l1[indy, 2]  # column index of pixel
# rat[rp, rc]
...
while (up_samp != 0) {  # while the number of samples to allocate is greater than 0
  ...
  # if (sn > 3) { sn <- 3 } # not more than 3 samples from one bin
...
#density
observations_hypercube_updated[which(h.mat.updated == 0)] <- 0.000001
observations_hypercube_updated
dat_dens <- observations_hypercube_updated / nrow(observations_df)
